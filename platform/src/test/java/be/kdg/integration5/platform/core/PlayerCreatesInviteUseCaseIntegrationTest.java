...
public class PlayerCreatesInviteUseCaseIntegrationTest extends AbstractDatabaseTest{
    ...
    public void inviteShouldBeCreated() {
        // ACT
        Invite invite = playerCreatesInviteUseCase.createInvite(PLAYER1_ID, PLAYER2_ID, OPEN_LOBBY_ID);
        ...
        assertEquals(inviteJpa.getSender().getPlayerId(), PLAYER1_ID);
        assertEquals(inviteJpa.getRecipient().getPlayerId(), PLAYER2_ID);
        assertEquals(inviteJpa.getLobby().getId(), OPEN_LOBBY_ID);
    }
}
