...
# Aggregate features
player_game_aggregates = merged.groupby(['player_id', 'game_name']).agg(
...
game_aggregates = merged.groupby('game_name').agg(
    game_avg_time=('match_duration', 'mean'),
    game_total_matches=('match_id', 'count')
).reset_index()

# Merge aggregates back
merged = merged.merge(player_game_aggregates, on=['player_id', 'game_name'], how='left')
merged = merged.merge(game_aggregates, on='game_name', how='left')
...
merged['target'] = merged.groupby(['player_id', 'game_name', 'date'])['match_duration'].transform('sum')

# Drop duplicate records for player_id, game_id, and date to create unique inputs
unique_data = merged[['player_id', 'game_name', 'date', 'day_of_week', 'is_weekend', 'avg_time_spent', 'total_time_spent', 'matches_played', 'game_avg_time', 'game_total_matches', 'age', 'gender_FEMALE', 'gender_MALE', 'target']].drop_duplicates()
...
agg_stats = merged.loc[train_records_index,].groupby(['player_id', 'game_name']).agg(
