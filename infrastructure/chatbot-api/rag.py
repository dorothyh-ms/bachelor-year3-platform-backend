...
def process_input(user_id: UUID, question: str, game: str):
    ...
    if answer.split(' ')[0].strip() in "True":
        ...
        Answer the question based only on the following context, the conversation history, and the content of the file {game}_rules.json only if the question is based on the boardGame do not mention the document in your answer:
        Context: {context}
        Game: {game}
        Question: {question}
        """
